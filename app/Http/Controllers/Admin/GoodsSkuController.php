...
class GoodsSkuController extends Controller
{
    ...
    public function edit($goodsId)
    {
        ...
        return  view('admin.goodsSku.add', $assign);
    }
    //执行保存的操作
    public function doAdd(Request $request)
    {
        ...
        try{
            ...
            $goodsSku = new GoodsSku();
            //先删除之前关联过得数据
            $this->delRecord($goodsSku,$params['goods_id'],'goods_id');
            $sku_arr = [];
            //添加spu的数据
            ...
            $this->addDataMany($goodsSku, $sku_arr);
            //添加sku的数据
            $sku1_arr = [];
            ...
            $this->addDataMany($goodsSku, $sku1_arr);
            DB::commit();
        }catch(\Exception $e){
            DB::rollback();
